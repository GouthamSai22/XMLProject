...
class Conceptizer(nn.Module):
    def __init__(self, input_dim, concept_num, concept_dim=None, num_channel=1):
        ...
        self.learnable = True
        self.add_bias = False
        self.output_dim = int(np.sqrt(input_dim) // 4)

        # Encoding
        self.conv1 = nn.Conv2d(num_channel, 64, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(128, concept_num, kernel_size=3, padding=1)
        self.linear = nn.Linear(self.output_dim ** 2, self.concept_dim)

        # Decoding
        self.unlinear = nn.Linear(self.concept_dim, self.output_dim ** 2)        
        self.deconv2 = nn.ConvTranspose2d(self.concept_num, 32, kernel_size=4, stride=2, padding=1)
        self.deconv1 = nn.ConvTranspose2d(32, num_channel, kernel_size=4, stride=2, padding=1)

    def encode(self, x):
        p = F.relu(self.conv1(x))
        p = F.relu(F.max_pool2d(self.conv2(p), 2))
        p = F.relu(F.max_pool2d(self.conv3(p), 2))
...
class Parametrizer(nn.Module):
    def __init__(self, input_dim, concept_num, output_dim, num_channel=1, only_positive=False):
        ...
        self.output_dim_conv = int(np.sqrt(input_dim) // 4)
        self.conv3_drop = nn.Dropout2d()
        self.fc1 = nn.Linear(128 * (self.output_dim_conv ** 2), concept_num * output_dim)
        self.fc2 = nn.Linear(concept_num * output_dim, concept_num * output_dim)
        self.positive = only_positive

    def forward(self, x):
        p = F.relu(F.max_pool2d(self.conv1(x), 2))
        p = F.relu(F.max_pool2d(self.conv2(p), 2))
        p = F.relu(self.conv3_drop(self.conv3(p)))
        p = p.view(-1, 128 * (self.output_dim_conv ** 2))
        p = F.relu(self.fc1(p))
        p = self.fc2(p)
